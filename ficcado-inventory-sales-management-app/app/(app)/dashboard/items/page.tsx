 ...
 * Items Management — full CRUD for item catalog and pricing.
 * Supports Create, Read, Update, Delete with modals.
 */
...
export default function ItemsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!newPrice || parseFloat(newPrice) <= 0) { setCreateError('Price must be greater than 0'); return; }
    if (newSizes.length === 0) { setCreateError('Select at least one size'); return; }

    setCreating(true);
    try {
      const res  = await fetch('/api/items', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ itemName: newName, itemType: newType, priceOfItem: parseFloat(newPrice), availableSizes: newSizes, currentStatus: newStatus }) });
      const data = await res.json();
      if (!res.ok) { setCreateError(data.error || 'Failed to create item'); return; }
      setShowCreate(false);
      setNewName(''); setNewPrice(''); setNewSizes(['S', 'M', 'L', 'XL']);
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    if (!editPrice || parseFloat(editPrice) <= 0) { setEditError('Price must be greater than 0'); return; }
    if (editSizes.length === 0) { setEditError('Select at least one size'); return; }

    setSaving(true);
    try {
      const res  = await fetch(`/api/items/${encodeURIComponent(editItem.sno)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ itemName: editName, itemType: editType, priceOfItem: parseFloat(editPrice), availableSizes: editSizes, currentStatus: editStatus }) });
      const data = await res.json();
